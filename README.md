# NFADC400_VME : Advanced VME FADC400 & DCS Control Framework
...
NFADC400_VME는 Notice Korea의 6UVME 및 NFADC400 모듈을 제어하기 위한 고성능 C++17 / ROOT 6 기반 데이터 획득(DAQ) 시스템이자, CAEN 고전압 장비(HV) 제어 및 자동화 시퀀서를 완벽하게 통합한 입자물리 실험실용 종합 통제 시스템(All-in-One Slow Control System)입니다.

## Acknowledgments & Official Maintenance
본 저장소(Repository)에는 하드웨어 제조사(Notice Korea)가 제공한 원본 드라이버, 라이브러리 및 기초 테스트 코드가 포함되어 있습니다. 제조사 측의 공식적인 동의 하에 소스코드가 통합되었으며, 기존 단일 스레드 기반의 획득 코드를 현대적인 멀티스레드 및 GUI 환경으로 전면 재설계(Rewrite)하였습니다. 본 저장소는 향후 NFADC400 모듈을 사용하는 연구자들을 위한 사실상의 표준(De facto standard) 프레임워크로서 지속 유지보수될 예정입니다.
...
- Trigger Logic Visualizer: 연구원을 위해 직관적인 ASCII 전자 회로도(Schematic Art)를 제공합니다. 드롭다운에서 원하는 동시계수(Coincidence) 물리 논리를 선택하면, FPGA 내부 진리표(Truth Table) 연산을 통해 설정 파일에 기입할 정확한 16진수 TLT 코드(ex: `0xFFFE`)를 자동으로 계산해 줍니다.
...
│
├── nfadc400/             # [제조사 원본] Notice Korea 공식 라이브러리 및 기초 테스트 코드
│
...
### 3. 파이썬 의존성 설치 및 GUI 가동
GUI 구동에 필요한 필수 파이썬 패키지를 설치합니다. OS 시스템 환경 보호를 위해 가상환경(`venv`) 구축을 권장합니다.
```bash
cd ..
...
# 2. 필수 패키지 설치 (GUI 프레임워크 및 CAEN HV 제어 라이브러리)
pip install PyQt5 caen-libs
